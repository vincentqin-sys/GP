...
// K线图属性
var K_MARGIN = 4, K_WIDTH = 8; // 每个k线图间间距  宽度
...
function getKColor(tag) {
    ...
    if (tag == 'Z')
        return "rgb(253,16,80)";
    // tag is 'D'
    return "rgb(13,244,155)"
}
...
function drawZBRate() {
    ...
    for (var i = 0; i < kBarsNum; i++) {
        var data = dataArr[i];
        ctx.beginPath();
        color = data.close >= data.open ? "rgb(253,16,80)" : "rgb(13,244,155)";
    ...
    ctx.moveTo(CANVAS_LR_MARGIN, zbY - ((ZB_HEIGHT - ZB_TOP_PADDING) * 5 / maxRate));
    ctx.lineTo(CANVAS_LR_MARGIN + canvasWidth, zbY - ((ZB_HEIGHT - ZB_TOP_PADDING) * 5 / maxRate));
    ctx.stroke();
    ctx.closePath();
    ...
    ctx.moveTo(CANVAS_LR_MARGIN, zbY - ((ZB_HEIGHT - ZB_TOP_PADDING) * 10 / maxRate));
    ctx.lineTo(CANVAS_LR_MARGIN + canvasWidth, zbY - ((ZB_HEIGHT - ZB_TOP_PADDING) * 10 / maxRate));
    ctx.stroke();
    ctx.closePath();
    ...
    if (maxRate >= 20) {
        ...
        ctx.moveTo(CANVAS_LR_MARGIN, zbY - ((ZB_HEIGHT - ZB_TOP_PADDING) * 20 / maxRate));
        ctx.lineTo(CANVAS_LR_MARGIN + canvasWidth, zbY - ((ZB_HEIGHT - ZB_TOP_PADDING) * 20 / maxRate));
        ctx.stroke();
        ctx.closePath();
    }
}
