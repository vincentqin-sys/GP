...
*/
    ...
    public static var sFile: System.IO.StreamWriter = null;
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        var holder: System.String = "basicapi/holder/stock/org_holder/rate?code=";
        if (url.Contains(holder)) {
            tag = "主力持仓";  // 主力持仓
            url = url.Substring(holder.Length);
            code = url.Substring(0, 6);
        } else {
            code = url.Substring(0, 6);
            if (code.Length != 6) {
                return;
            }
            // check code is number
            for (var i = 0; i < 6; ++i) {
                if (! System.Char.IsDigit(code[i])) {
                    return;
                }
            }
        ...
        if (code != "" && tag != "") {
            ...
            oSession.SaveResponseBody("D:\\VSCode\\THS\\" + code + tag);
        }
    }
